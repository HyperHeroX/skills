...
function pass(detail = '', details = []) {
  return { code: 'CS3230300E', rule: '使用CSS媒體查詢的reduce-motion避免畫面動態', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'CS3230300E', rule: '使用CSS媒體查詢的reduce-motion避免畫面動態', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // CSS 檢查：使用CSS媒體查詢的reduce-motion避免畫面動態
    ...
    return { code: 'CS3230300E', rule: '使用CSS媒體查詢的reduce-motion避免畫面動態', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AAA',
  category: 'CSS',
  rule: '使用CSS媒體查詢的reduce-motion避免畫面動態',
};
