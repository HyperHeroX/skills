...
function pass(detail = '', details = []) {
  return { code: 'GN2130400E', rule: '允許使用者可以使用不同方向操作控制元件', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN2130400E', rule: '允許使用者可以使用不同方向操作控制元件', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  // 通用 DOM 查詢：允許使用者可以使用不同方向操作控制元件
    ...
    return { code: 'GN2130400E', rule: '允許使用者可以使用不同方向操作控制元件', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
