...
function pass(detail = '', details = []) {
  return { code: 'GN2330401E', rule: '在法律、財務、個人資料，均提供表單遞交後何時可由使用者更新或取消的時段說明', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN2330401E', rule: '在法律、財務、個人資料，均提供表單遞交後何時可由使用者更新或取消的時段說明', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：在法律、財務、個人資料，均提供表單遞交後何時可由使用者更新或取消的時段說明
    ...
    return { code: 'GN2330401E', rule: '在法律、財務、個人資料，均提供表單遞交後何時可由使用者更新或取消的時段說明', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AA',
  category: 'General',
  rule: '在法律、財務、個人資料，均提供表單遞交後何時可由使用者更新或取消的時段說明',
};
