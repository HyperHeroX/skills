...
function pass(detail = '', details = []) {
  return { code: 'CS2141200E', rule: '允許使用者按照其偏好覆蓋原有的網頁文字設定間距', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'CS2141200E', rule: '允許使用者按照其偏好覆蓋原有的網頁文字設定間距', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    return { code: 'CS2141200E', rule: '允許使用者按照其偏好覆蓋原有的網頁文字設定間距', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
