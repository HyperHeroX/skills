...
function pass(detail = '', details = []) {
  return { code: 'GN1330200E', rule: '在使用者送出資料前，先描述會發生什麼事', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1330200E', rule: '在使用者送出資料前，先描述會發生什麼事', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  // 通用 DOM 查詢：在使用者送出資料前，先描述會發生什麼事
    ...
    return { code: 'GN1330200E', rule: '在使用者送出資料前，先描述會發生什麼事', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
