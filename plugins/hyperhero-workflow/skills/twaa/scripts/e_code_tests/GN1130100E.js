...
function pass(detail = '', details = []) {
  return { code: 'GN1130100E', rule: '使用文字來傳達藉由文字呈現上的變化所傳達的資訊', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1130100E', rule: '使用文字來傳達藉由文字呈現上的變化所傳達的資訊', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：使用文字來傳達藉由文字呈現上的變化所傳達的資訊
    ...
    return { code: 'GN1130100E', rule: '使用文字來傳達藉由文字呈現上的變化所傳達的資訊', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
