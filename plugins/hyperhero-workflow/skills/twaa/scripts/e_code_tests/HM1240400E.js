...
function pass(detail = '', details = []) {
  return { code: 'HM1240400E', rule: '使用鏈結文字及前後的脈絡情境來指明鏈結目的', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'HM1240400E', rule: '使用鏈結文字及前後的脈絡情境來指明鏈結目的', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    return { code: 'HM1240400E', rule: '使用鏈結文字及前後的脈絡情境來指明鏈結目的', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
