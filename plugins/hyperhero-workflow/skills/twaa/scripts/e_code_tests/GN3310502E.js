...
function pass(detail = '', details = []) {
  return { code: 'GN3310502E', rule: '提供視覺插圖、圖片、符號以協助解釋概念、事件、程序。', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN3310502E', rule: '提供視覺插圖、圖片、符號以協助解釋概念、事件、程序。', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：提供視覺插圖、圖片、符號以協助解釋概念、事件、程序。
    ...
    return { code: 'GN3310502E', rule: '提供視覺插圖、圖片、符號以協助解釋概念、事件、程序。', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AAA',
  category: 'General',
  rule: '提供視覺插圖、圖片、符號以協助解釋概念、事件、程序。',
};
