...
function pass(detail = '', details = []) {
  return { code: 'GN2140301E', rule: '確認大尺寸文字(及影像文字)與文字後面的背景間，至少有3:1的對比值', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN2140301E', rule: '確認大尺寸文字(及影像文字)與文字後面的背景間，至少有3:1的對比值', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：確認大尺寸文字(及影像文字)與文字後面的背景間，至少有3:1的對比值
    ...
    return { code: 'GN2140301E', rule: '確認大尺寸文字(及影像文字)與文字後面的背景間，至少有3:1的對比值', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AA',
  category: 'General',
  rule: '確認大尺寸文字(及影像文字)與文字後面的背景間，至少有3:1的對比值',
};
