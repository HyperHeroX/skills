...
function pass(detail = '', details = []) {
  return { code: 'GN1330205E', rule: '在表單或一組表單欄位開頭處提供文字指示來描述必要的輸入欄位', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1330205E', rule: '在表單或一組表單欄位開頭處提供文字指示來描述必要的輸入欄位', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：在表單或一組表單欄位開頭處提供文字指示來描述必要的輸入欄位
    ...
    return { code: 'GN1330205E', rule: '在表單或一組表單欄位開頭處提供文字指示來描述必要的輸入欄位', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'A',
  category: 'General',
  rule: '在表單或一組表單欄位開頭處提供文字指示來描述必要的輸入欄位',
};
