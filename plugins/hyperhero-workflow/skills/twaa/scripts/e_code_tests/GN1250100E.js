...
function pass(detail = '', details = []) {
  return { code: 'GN1250100E', rule: '提供單點指標控制元件以實現與基於路徑或多點手勢相同的結果', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1250100E', rule: '提供單點指標控制元件以實現與基於路徑或多點手勢相同的結果', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：提供單點指標控制元件以實現與基於路徑或多點手勢相同的結果
    ...
    return { code: 'GN1250100E', rule: '提供單點指標控制元件以實現與基於路徑或多點手勢相同的結果', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'A',
  category: 'General',
  rule: '提供單點指標控制元件以實現與基於路徑或多點手勢相同的結果',
};
