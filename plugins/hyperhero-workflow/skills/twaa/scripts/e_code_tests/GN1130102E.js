...
function pass(detail = '', details = []) {
  return { code: 'GN1130102E', rule: '從呈現當中抽離資訊與結構，以便啟用不同的呈現', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1130102E', rule: '從呈現當中抽離資訊與結構，以便啟用不同的呈現', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：從呈現當中抽離資訊與結構，以便啟用不同的呈現
    ...
    return { code: 'GN1130102E', rule: '從呈現當中抽離資訊與結構，以便啟用不同的呈現', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
