...
function pass(detail = '', details = []) {
  return { code: 'GN1220100E', rule: '在多頁表單的第1頁提供一個核選框，讓使用者可以要求較寬鬆的階段時間限制，或者完全不要有階段時間限制', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1220100E', rule: '在多頁表單的第1頁提供一個核選框，讓使用者可以要求較寬鬆的階段時間限制，或者完全不要有階段時間限制', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  const formResult = await page.evaluate(() => {
    ...
    return { code: 'GN1220100E', rule: '在多頁表單的第1頁提供一個核選框，讓使用者可以要求較寬鬆的階段時間限制，或者完全不要有階段時間限制', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'A',
  category: 'General',
  rule: '在多頁表單的第1頁提供一個核選框，讓使用者可以要求較寬鬆的階段時間限制，或者完全不要有階段時間限制',
};
