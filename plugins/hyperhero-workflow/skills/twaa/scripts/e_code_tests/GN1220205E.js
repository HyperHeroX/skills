...
function pass(detail = '', details = []) {
  return { code: 'GN1220205E', rule: '動畫GIF圖檔之閃動時間不超過五秒鐘', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1220205E', rule: '動畫GIF圖檔之閃動時間不超過五秒鐘', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  // 通用 DOM 查詢：動畫GIF圖檔之閃動時間不超過五秒鐘
    ...
    return { code: 'GN1220205E', rule: '動畫GIF圖檔之閃動時間不超過五秒鐘', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
