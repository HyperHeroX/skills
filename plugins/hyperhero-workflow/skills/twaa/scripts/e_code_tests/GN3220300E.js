...
function pass(detail = '', details = []) {
  return { code: 'GN3220300E', rule: '讓使用者可以在沒有任何時間限制的情況下完成活動。', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN3220300E', rule: '讓使用者可以在沒有任何時間限制的情況下完成活動。', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：讓使用者可以在沒有任何時間限制的情況下完成活動。
    ...
    return { code: 'GN3220300E', rule: '讓使用者可以在沒有任何時間限制的情況下完成活動。', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
