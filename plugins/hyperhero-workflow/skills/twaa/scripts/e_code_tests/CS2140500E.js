...
function pass(detail = '', details = []) {
  return { code: 'CS2140500E', rule: '使用CSS來將文字取代成影像文字，並提供使用者介面控制元件來加以切換', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'CS2140500E', rule: '使用CSS來將文字取代成影像文字，並提供使用者介面控制元件來加以切換', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // CSS 檢查：使用CSS來將文字取代成影像文字，並提供使用者介面控制元件來加以切換
    ...
    return { code: 'CS2140500E', rule: '使用CSS來將文字取代成影像文字，並提供使用者介面控制元件來加以切換', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AA',
  category: 'CSS',
  rule: '使用CSS來將文字取代成影像文字，並提供使用者介面控制元件來加以切換',
};
