/**
 * GN2330403E — 在法律、財務、個人資料方面，均要求確認後才繼續所選的動作
 * 對應成功準則：3.3.4（等級 AA）
 * 類別：General
 *
 * 官方稽核步驟：
...
function pass(detail = '', details = []) {
  return { code: 'GN2330403E', rule: '在法律、財務、個人資料方面，均要求確認後才繼續所選的動作', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN2330403E', rule: '在法律、財務、個人資料方面，均要求確認後才繼續所選的動作', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：在法律、財務、個人資料方面，均要求確認後才繼續所選的動作
    ...
    return { code: 'GN2330403E', rule: '在法律、財務、個人資料方面，均要求確認後才繼續所選的動作', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AA',
  category: 'General',
  rule: '在法律、財務、個人資料方面，均要求確認後才繼續所選的動作',
};
