...
function pass(detail = '', details = []) {
  return { code: 'GN1220204E', rule: '提供鏈結、按鈕，或任何可以不閃動內容即重新載入頁面的機制', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1220204E', rule: '提供鏈結、按鈕，或任何可以不閃動內容即重新載入頁面的機制', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：提供鏈結、按鈕，或任何可以不閃動內容即重新載入頁面的機制
    ...
    return { code: 'GN1220204E', rule: '提供鏈結、按鈕，或任何可以不閃動內容即重新載入頁面的機制', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'A',
  category: 'General',
  rule: '提供鏈結、按鈕，或任何可以不閃動內容即重新載入頁面的機制',
};
