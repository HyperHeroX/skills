...
function pass(detail = '', details = []) {
  return { code: 'GN1110110E', rule: '任何CAPTCHA驗證均需提供描述CAPTCHA驗證目的的替代文字', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1110110E', rule: '任何CAPTCHA驗證均需提供描述CAPTCHA驗證目的的替代文字', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：任何CAPTCHA驗證均需提供描述CAPTCHA驗證目的的替代文字
    ...
    return { code: 'GN1110110E', rule: '任何CAPTCHA驗證均需提供描述CAPTCHA驗證目的的替代文字', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'A',
  category: 'General',
  rule: '任何CAPTCHA驗證均需提供描述CAPTCHA驗證目的的替代文字',
};
