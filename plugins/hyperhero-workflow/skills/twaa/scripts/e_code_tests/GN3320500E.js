...
function pass(detail = '', details = []) {
  return { code: 'GN3320500E', rule: '除非使用即刻用戶端重新導向，否則不要自動更新內容，而是提供可以請求內容更新的機制。', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN3320500E', rule: '除非使用即刻用戶端重新導向，否則不要自動更新內容，而是提供可以請求內容更新的機制。', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：除非使用即刻用戶端重新導向，否則不要自動更新內容，而是提供可以請求內容更新的機制。
    ...
    return { code: 'GN3320500E', rule: '除非使用即刻用戶端重新導向，否則不要自動更新內容，而是提供可以請求內容更新的機制。', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AAA',
  category: 'General',
  rule: '除非使用即刻用戶端重新導向，否則不要自動更新內容，而是提供可以請求內容更新的機制。',
};
