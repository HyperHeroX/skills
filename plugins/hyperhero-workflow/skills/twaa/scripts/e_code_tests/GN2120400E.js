...
function pass(detail = '', details = []) {
  return { code: 'GN2120400E', rule: '為現場的同步媒體建立字幕', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN2120400E', rule: '為現場的同步媒體建立字幕', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  const result = await page.evaluate(() => {
    ...
    return { code: 'GN2120400E', rule: '為現場的同步媒體建立字幕', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
