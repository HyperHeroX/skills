...
function pass(detail = '', details = []) {
  return { code: 'HM2130500E', rule: '使用HTML 5.2自動完成之屬性', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'HM2130500E', rule: '使用HTML 5.2自動完成之屬性', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  // 通用 DOM 查詢：使用HTML 5.2自動完成之屬性
    ...
    return { code: 'HM2130500E', rule: '使用HTML 5.2自動完成之屬性', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
