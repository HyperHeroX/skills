...
function pass(detail = '', details = []) {
  return { code: 'GN3330600E', rule: '在所有情況中，均提供取回已刪除資訊的能力', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN3330600E', rule: '在所有情況中，均提供取回已刪除資訊的能力', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：在所有情況中，均提供取回已刪除資訊的能力
    ...
    return { code: 'GN3330600E', rule: '在所有情況中，均提供取回已刪除資訊的能力', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
