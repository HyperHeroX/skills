...
function pass(detail = '', details = []) {
  return { code: 'CS2141002E', rule: '針對長網址跟文字字符串可以進行重排', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'CS2141002E', rule: '針對長網址跟文字字符串可以進行重排', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  // CSS 檢查：針對長網址跟文字字符串可以進行重排
    ...
    return { code: 'CS2141002E', rule: '針對長網址跟文字字符串可以進行重排', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
