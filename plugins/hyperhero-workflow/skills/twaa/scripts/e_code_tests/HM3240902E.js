...
function pass(detail = '', details = []) {
  return { code: 'HM3240902E', rule: '針對單獨存在的鏈結，提供描述鏈結組件鏈結目的的鏈結文字', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'HM3240902E', rule: '針對單獨存在的鏈結，提供描述鏈結組件鏈結目的的鏈結文字', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：針對單獨存在的鏈結，提供描述鏈結組件鏈結目的的鏈結文字
    ...
    return { code: 'HM3240902E', rule: '針對單獨存在的鏈結，提供描述鏈結組件鏈結目的的鏈結文字', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AAA',
  category: 'HTML',
  rule: '針對單獨存在的鏈結，提供描述鏈結組件鏈結目的的鏈結文字',
};
