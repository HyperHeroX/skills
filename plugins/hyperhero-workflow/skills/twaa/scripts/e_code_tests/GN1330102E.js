...
function pass(detail = '', details = []) {
  return { code: 'GN1330102E', rule: '資料成功送出後，提供成功的回饋', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN1330102E', rule: '資料成功送出後，提供成功的回饋', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  const formResult = await page.evaluate(() => {
    ...
    return { code: 'GN1330102E', rule: '資料成功送出後，提供成功的回饋', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
