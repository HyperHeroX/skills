...
function pass(detail = '', details = []) {
  return { code: 'CS2141001E', rule: '使用CSS彈性容器重排內容', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'CS2141001E', rule: '使用CSS彈性容器重排內容', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  // CSS 檢查：使用CSS彈性容器重排內容
    ...
    return { code: 'CS2141001E', rule: '使用CSS彈性容器重排內容', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
