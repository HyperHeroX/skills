...
function pass(detail = '', details = []) {
  return { code: 'GN3230200E', rule: '確認在任何一秒鐘的週期內，沒有任何內容標籤會閃爍超過三次。', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'GN3230200E', rule: '確認在任何一秒鐘的週期內，沒有任何內容標籤會閃爍超過三次。', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：確認在任何一秒鐘的週期內，沒有任何內容標籤會閃爍超過三次。
    ...
    return { code: 'GN3230200E', rule: '確認在任何一秒鐘的週期內，沒有任何內容標籤會閃爍超過三次。', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
...
export const metadata = {
  ...
  level: 'AAA',
  category: 'General',
  rule: '確認在任何一秒鐘的週期內，沒有任何內容標籤會閃爍超過三次。',
};
