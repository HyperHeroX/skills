...
function pass(detail = '', details = []) {
  return { code: 'CS2141007E', rule: '使用CSS最大寬度和高度容納圖像', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'CS2141007E', rule: '使用CSS最大寬度和高度容納圖像', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  ...
  // CSS 檢查：使用CSS最大寬度和高度容納圖像
    ...
    return { code: 'CS2141007E', rule: '使用CSS最大寬度和高度容納圖像', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
