...
function pass(detail = '', details = []) {
  return { code: 'HM1130109E', rule: '以有意義的標記來建立表格標題與資料表格的關連', status: 'pass', message: detail, fix_suggestion: '', details };
}
function fail(message, fix = '', details = []) {
  return { code: 'HM1130109E', rule: '以有意義的標記來建立表格標題與資料表格的關連', status: 'fail', message, fix_suggestion: fix, details: Array.isArray(details) ? details : [details] };
}
...
export async function check(page) {
  try {
  // 通用 DOM 查詢：以有意義的標記來建立表格標題與資料表格的關連
    ...
    return { code: 'HM1130109E', rule: '以有意義的標記來建立表格標題與資料表格的關連', status: 'fail', message: `執行錯誤: ${err.message}`, fix_suggestion: '確認頁面載入完成後再執行', details: [] };
  }
}
